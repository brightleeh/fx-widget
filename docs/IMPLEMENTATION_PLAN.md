...
- Milestone 6 is in progress: a shared `WidgetConfigurationSelectionPolicy` validates provider support, active-reference exclusion, duplicates, and fixed family capacity without truncating existing selections. App Intents-compatible optional family collections initialize BIS defaults, migrate a missing legacy value to the concrete family default while preserving an explicit empty selection, allow deletion to zero, and use limits 3/10/20. Reference and collection entity restoration no longer depends on a successful provider call. Reference changes create distinct keyed requests, and the header identifies the configured reference with the same compact Currency Name style used by rows. Columns, Text Size, and separate Country Names have been removed; `Currency Name` defaults to On for fresh widgets. Post-install edit-screen validation remains required.
...
- the WidgetKit recommendation persists those defaults into the editable family-specific collection,
...
Status: in progress. Core selection/addition policy, reference-currency swap, active-reference candidate filtering, family-specific editable BIS defaults, missing-versus-explicit-empty handling, and fixed family bounds are implemented. The installed ad-hoc host must be revalidated for edit-time default visibility, deletion, reorder behavior, reference changes, and exact 3/10/20 capacity rejection.
